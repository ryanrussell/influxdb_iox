        ...
        + Sharder<DeletePredicate, Item = Vec<Arc<Sequencer>>>,
{
    ...
    async fn write(
        ...
        let iter = collated.into_iter().map(|(sequencer, batch)| {
            ...
            trace!(
                %partition_key,
                kafka_partition=%sequencer.kafka_partition(),
    ...
    async fn delete(
        ...
        let iter = sequencers.into_iter().map(|s| {
            trace!(sequencer_id=%s.kafka_partition(), %table_name, %namespace, "routing delete to shard");
...
mod tests {
    ...
    async fn test_shard_writes_collated() {
        ...
        let mut got = write_buffer_state.get_messages(shard.kafka_partition() as _);
        assert_eq!(got.len(), 1);
        let got = got
            .pop()
            .unwrap()
    ...
    async fn test_multiple_shard_writes() {
        ...
        let mut got = write_buffer1_state.get_messages(shard1.kafka_partition() as _);
        assert_eq!(got.len(), 1);
        let got = got
            .pop()
            .unwrap()
        ...
        let mut got = write_buffer2_state.get_messages(shard2.kafka_partition() as _);
        assert_eq!(got.len(), 1);
        let got = got
            .pop()
            .unwrap()
    ...
    async fn test_write_partial_success() {
        ...
        let got = write_buffer1_state.get_messages(shard1.kafka_partition() as _);
        assert_eq!(got.len(), 1);
    }
